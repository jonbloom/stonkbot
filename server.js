  ...
  wsClinet.on('connect', connection => {
    ...
    Tickers.findAll().then(tickers => {
      tickers.forEach(ticker => {
        ...
        connection.send(JSON.stringify({ 'type': 'subscribe', 'symbol': ticker.symbol }))
      });
    });
    connection.on('message', e => {
      const { data } = JSON.parse(e.utf8Data);
      if (data) {
        data.sort((a, b) => a.t - b.t).forEach(dataPoint => {
    ...
    client.on('message', async message => {
      if (message.content.startsWith(PREFIX) && message.author.id == "387722383567618051") {
        ...
        if (command == "add"){
          ...
          Tickers.destroy({
            ...
            connection.send(JSON.stringify({ 'type': 'unsubscribe', 'symbol': toDelete }))
    ...
    setInterval(() => {
      ...
      Tickers.findAll({
        ...
        tickers.forEach((ticker, i) => {
          ...
          toSend += `\`${directions[ticker.symbol]} ${ticker.symbol} ${formatter.format(ticker.lastPrice)}\``;
